...
During compilation, it downloads iotivity from its git repository, builds it, and links against it. If the environment variable ```EXTERNAL_OCTBSTACK``` is non-empty during compilation, it attempts to retrieve iotivity compiler and linker flags from the environment variables ```OCTBSTACK_CFLAGS``` for compiler flags and ```OCTBSTACK_LIBS``` for the linker. If you choose to build iotivity by hand, you can use the following procedure, which is known to work on Linux:
...
0. Set the following environment variables:
	* ```EXTERNAL_OCTBSTACK``` - this variable should be present to indicate that building iotivity is to be avoided.
...
Alternatively, you can use some rudimentary install scripts for both iotivity and this repository. Using them will help you avoid having to set the environment variables ```OCTBSTACK_CFLAGS``` and ```OCTBSTACK_LIBS```. You must still set ```EXTERNAL_OCTBSTACK```.
