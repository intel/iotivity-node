...
_.extend( OicRequestEvent.prototype, {
	...
	sendResponse: function( resource ) {
		...
		/*
		// FIXME:
		// This didn't work as numRcvdVendorSpecificHeaderOptions wasn't UInt32
		var oicResponse = new Array();
		oicResponse.requestHandle = new Buffer(this.requestId);
		oicResponse.resourceHandle = new Buffer(this.target);
		...
		*/
		...
		ret = iotivity.OCDoResponse( {
			requestHandle: this.requestId,
			resourceHandle: this.target,
			ehResult: iotivity.OCEntityHandlerResult.OC_EH_OK,
			payload: payload,
			payloadSize: payload.length,
			numSendVendorSpecificHeaderOptions: this.headerOptions.length,
			sendVendorSpecificHeaderOptions: _.clone(this.headerOptions),
			//FIXME: This url should be a FQDN or COAP URL ?
...
_.extend( OicServer.prototype, {
	...
	onrequest: null,
	listeners: {},
	_interestedObservers: new Array(),
		...
		return new Promise( _.bind( function( fulfill, reject ) {
			...
			result = iotivity.OCCreateResource (
					...
					function ( flag, request ) {
						...
						oicReq.requestId = request.requestHandle.handle;
						oicReq.target = request.resource.handle;
						...
						if ( flag & iotivity.OCEntityHandlerFlag.OC_OBSERVE_FLAG ) {
							if ( request.obsInfo.action == iotivity.OCObserveAction.OC_OBSERVE_REGISTER ) {
								...
								var index = resource._server._interestedObservers.indexOf( request.obsInfo.obsId );
								//FIXME: Should we loop and remove?
								while ( index != -1 ) {
									resource._server._interestedObservers.splice ( index, 1 );
									index = resource._server._interestedObservers.indexOf( request.obsInfo.obsId );
								}
							}
