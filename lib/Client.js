...
// options: {
//   resource:     object -> attempt to use endpoint information
//                 absent -> fall back on deviceId and destination
...
function oneShotRequest( options ) {
	return new Promise( function( fulfill, reject ) {
		...
		// Use endpoint information if available.
		if ( options.resource ) {
			if ( options.resource._private && options.resource._private.origin ) {
				deviceId = null;
				requestUri = options.resource._private.origin + options.resource.resourcePath;
			} else {
				deviceId = options.resource.deviceId;
				requestUri = options.resource.resourcePath;
			}
		}

		// If a deviceId is given, we need to convert that to an OCDevAddr.
		if ( deviceId ) {
			destination = typeof deviceId === "object" ?
				deviceId : resolver.get( deviceId );
