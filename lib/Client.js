...
client = _.extend( new Client(), {
	...
	retrieve: function( resourceId, query, listener, removeListener ) {
		...
		return Promise.resolve( ( resourceId instanceof ClientResource ) ?
			...
			.then( function( resource ) {
				...
				return removeListener ?
					new Promise( function removeListener( fulfill ) {
						if ( listener ) {
							resource.removeListener( "_update", listener );
						} else {
							resource.removeAllListeners( "_update" );
						}
						fulfill( resource );
					} ) : listener ?
					new Promise( function addListener( fulfill, reject ) {
						var result = ClientResource.observe( resource, fulfill, reject );

						if ( !( result instanceof Error ) ) {
							resource.on( "_update", listener );
						}
