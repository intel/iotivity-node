...
function oneShotRequest( options ) {
	return new Promise( function( fulfill, reject ) {
		...
		if ( options.resource ) {
			destination = ClientResource.getDestination( options.resource );
		}

		// If a deviceId is given, we need to convert that to an OCDevAddr.
		if ( destination.deviceId ) {
			deviceAddress = typeof destination.deviceId === "object" ?
				destination.deviceId : resolver.get( destination.deviceId );
			if ( deviceAddress instanceof Error ) {
				reject( deviceAddress );
				return;
			}
		}
