...
var establishDestination = require( "./establishDestination" );
...
function finalizeResource( resource ) {
	...
	resource.removeAllListeners( "_update" );
...
var ClientResource = function ClientResource( init, forceNew ) {
	...
	_.extend( this,
		...
		_.pick( init, [
			...
			"discoverable", "slow", "secure", "observable", "active", "endpoints", "endpoint"
		] ) );

	this.on( "newListener", function( eventName, listener ) {
		if ( eventName === "_update" && !this._private.observer ) {
			...
			if ( result instanceof Error ) {
				setImmediate( function( emitter, listener ) {
					emitter.removeListener( "_update", listener );
				}, this, listener );
	...
	this.on( "removeListener", function( eventName ) {
		if ( eventName === "_update" &&
				listenerCount( this, "_update" ) === 0 &&
				this._private.observer ) {
			result =
				handles.replace( this._private.observeParams, this._private.observer, "remove" );
			if ( result instanceof Error ) {
				this.emit( "error", result );
			} else {
				delete this._private.observer;
				delete this._private.observeParams;
			}
		}
		if ( eventName === "delete" && listenerCount( this, "delete" ) === 0 ) {
			unwatchDelete( this );
...
ClientResource.observe = function( resource, fulfill, reject ) {
	...
	var updateResource = function( response ) {
		...
		if ( properties instanceof Error ) {
			...
			resource.emit( "_update", resource );
			if ( needsResolve ) {
				fulfill( resource );
			}
		}
	...
	if ( !resource._private.observer ) {
		savedObserveParams = observeParams( resource );
		result = handles.replace( savedObserveParams, updateResource, "add" );
		...
		if ( result instanceof Error ) {
			...
			resource._private.observeParams = savedObserveParams;
		}
	}
