...
_.extend( OicClient.prototype, {
	...
	findDevices: function( options ) {
		return new Promise( _.bind( function( fulfill, reject ) {
			...
			var ret = iotivity.OCDoResource( handle, iotivity.OCMethod.OC_REST_DISCOVER,
				...
				iotivity.OCQualityOfService.OC_HIGH_QOS, //FIXME: Checkon the QOS
				function ( handle, response ) {
					...
					//client._devices[0] = response.devAddr;

					client.dispatchEvent( "devicefound", event );
	...
	findResources: function( options ) {
		return new Promise( _.bind( function( fulfill, reject ) {
			...
			if ( options.resourceId && options.resourceId != "" ) {
				...
				if ( !resource ) {
					reject( _.extend( new Error( "Unable to find resource" ), {
												result: iotivity.OCStackApplicationResult.OC_STACK_NO_RESOURCE
											} ) );
					return;
				}
			...
			var ret = iotivity.OCDoResource( handle, iotivity.OCMethod.OC_REST_DISCOVER,
				...
				function ( handle, response ) {
					...
					for ( i=0; i<resourceCount; i++ ) {
						...
						var id = resource.host + ":" + resource.uri;
						//FIXME: Should we have resource ID if not there ?
						resource.id = id;
						//FIXME Should we copy using _. ?
						// We should create a deviceID and have it referenced that way
						client._devices[ id ] = devAddr;
						client._resources[ id ] = resource;
						var event = new OicResourceFoundEvent();
						event.resource = resource;
						client.dispatchEvent( "resourcefound" , event);
					}
			...
			timeout = setTimeout(function() {
				...
				//iotivity.OCCancel(handle.handle, iotivity.OCQualityOfService.OC_HIGH_QOS, null, 0);
				fulfill();
			}, waitsec*1000)
	...
	updateResource: function( resource ) {
		return new Promise( _.bind( function( fulfill, reject ) {
			...
			var handle = {};
			var rep = JSON.stringify( resource );
	...
	startObserving: function( id ) {
		return new Promise( _.bind( function( fulfill, reject ) {
			...
			var ret = iotivity.OCDoResource( handle, iotivity.OCMethod.OC_REST_OBSERVE,
				...
				function ( handle, response ) {
					if (response.payload) {
						var event = new OicResourceChangedEvent();
						event.resource = new OicResource( response.payload );
						// FIXME: We should return a resource?
						fulfill( event.resource );
						client.dispatchEvent( "resourcechange", event );
					}

					if( response.sequenceNumber == iotivity.OCObserveAction.OC_OBSERVE_DEREGISTER ) {
						//Should we do this?
						client._observationhandles[ id ] = null;
