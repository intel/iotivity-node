...
_.extend( devicePrototype, {
	...
	_processResources: function( response, pathFilter ) {
		...
		if ( resourceCount > 0 ) {
			var deviceId = response.payload.sid;
	...
	retrieve: function( id, queryParameters ) {
		return this._oneShotRequest( {
			...
			createAnswer: function retrieveResourceFromResponse( handle, response ) {
				...
				var properties = utils.payloadToObject( response.payload.values );

				if ( resource ) {
					_.extend( resource.properties, properties );
				} else {

					// If we haven't seen this resource before, it means we've discovered a
					// resource by "poking around in the dark" - i.e., we knew a priori that the
					// given device had a resource located at the given path.
					resource = this._resources[ id.deviceId + ":" + id.path ] =
						new OicResource( { id: id, properties: properties } );
