...
NAN_METHOD( bind_OCNotifyListOfObservers ) {
	...
	// If the array contains fewer IDs than the reported number, we use the lower number
	...
	if ( !c_observations && numberOfIds > 0 ) {
		NanThrowError( "OCNotifyListOfObservers: Failed to allocate list of observers" );
		NanReturnUndefined();
	}
	...
	Local<Number> returnValue = NanNew<Number>( OCNotifyListOfObservers(
		handle,
		c_observations,
		numberOfIds,
