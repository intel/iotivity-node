...
static void default_SetInputPinWithContextCB(OicUuid_t deviceId,
  ...
  if (!OCConvertUuidToString(deviceId.id, uuid_string)) {
    NAPI_CALL(cData->env,
        napi_throw_error(cData->env, "iotivity-node",
  ...
  napi_value argv[2];
  NAPI_CALL(cData->env,
      napi_create_string_utf8(cData->env, uuid_string, UUID_STRING_SIZE,
          &argv[0]),
      THROW_BODY(cData->env, ));
  NAPI_CALL(cData->env,
      napi_create_uint32(cData->env, pinBufferSize, &argv[1]),
      THROW_BODY(cData->env, ));
  napi_value js_cb;
  NAPI_CALL(cData->env,
      napi_get_reference_value(cData->env, cData->callback, &js_cb),
...
static void default_SetDisplayPinWithContextCB(char* pinData,
                                             size_t pinSize,
                                             void* context) {
  JSCallback* cData = static_cast<JSCallback*>(context);
  NapiHandleScope scope(cData->env);
...
napi_value bind_GeneratePin(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env,
      napi_get_buffer_info(env, arguments[0], &buffer_data, &buffer_length));
